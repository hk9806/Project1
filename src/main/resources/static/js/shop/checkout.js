...
(() => {
    ...
    async function updateSelectedSummary() {
        const selectedRadio  = $('input[name="addressId"]:checked');
        ...
        const shippingEl = $('#selectedShipping');
        ...
        try {
            ...
            if (totalEl)    totalEl.textContent    = payText;
...
(function(){
    ...
    function renderTotals(){
        ...
        if (server && Number.isFinite(server.productsTotal)) {
            ...
            const shippingEl = $('#selectedShipping');
            ship          = shippingEl ? onlyDigits(shippingEl.textContent) : 0;
            total         = products + gradeDiscount + ship;
        }

        const sumProducts   = $('#sumProducts');
        const gradeNode     = $('#gradeDiscount');
